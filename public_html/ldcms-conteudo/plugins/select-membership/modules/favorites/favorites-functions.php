<?php
if ( ! function_exists( 'qodef_membership_add_item_to_favorites' ) ) {
    function qodef_membership_add_item_to_favorites() {
        $user_id = get_current_user_id();

        if ( empty( $_POST ) || ! isset( $_POST ) ) {
            qodef_core_ajax_status( 'error', esc_html__( 'All fields are empty', 'select-membership' ) );
        } else {
            ...
            qodef_core_ajax_status( 'success', '', $data );
        }

        wp_die();
    }

    add_action( 'wp_ajax_qodef_membership_add_item_to_favorites', 'qodef_membership_add_item_to_favorites' );
}

if ( ! function_exists( 'qodef_membership_is_item_in_favorites' ) ) {
    function qodef_membership_is_item_in_favorites( $item_id = '', $user_id = '' ) {
...
if ( ! function_exists( 'qodef_membership_get_number_of_item_favorites' ) ) {
    function qodef_membership_get_number_of_item_favorites( $item_id = '' ) {
        $item_id    = $item_id != '' ? $item_id : get_the_ID();
...
if ( ! function_exists( 'qodef_membership_get_user_favorites' ) ) {
    function qodef_membership_get_user_favorites( $user_id = '', $post_types = array() ) {
        $user_id                = $user_id != '' ? $user_id : get_current_user_id();
...
if ( ! function_exists( 'qodef_membership_get_favorite_template' ) ) {
    function qodef_membership_get_favorite_template($item_id = '', $template = 'icon', $args = array()) {
        if ( is_user_logged_in() ) {
            $item_id    = $item_id != '' ? $item_id : get_the_ID();
            if ( ! qodef_membership_is_item_in_favorites($item_id) ) {
                $text = sizeof(get_post_meta( $item_id, 'qodef_users_favorites', true ));
                $icon = 'fa-heart-o';
            } else {
                $text = sizeof(get_post_meta( $item_id, 'qodef_users_favorites', true ));
                $icon = 'fa-heart';
            ...
            print qodef_membership_get_module_template_part( 'favorites', 'favorite', $template, '', $favorites_params );
        }
    }
}