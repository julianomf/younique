...
(function ($) {
    ...
    function qodefUserLogin() {
        $('.qodef-login-form').on('submit', function (e) {
            e.preventDefault();
            var ajaxData = {
                action: 'qodef_membership_login_user',
                security: $(this).find('#qodef-login-security').val(),
                login_data: $(this).serialize()
            };
	        var message = $(this).find('#login-form-submitted').val();
	        var formSubmitParams = {status:"sent", message:message};
	        qodefRenderAjaxResponseMessage(formSubmitParams);
            $.ajax({
                type: 'POST',
                data: ajaxData,
    ...
    function qodefUserRegister() {
        ...
        $('.qodef-register-form').on('submit', function (e) {
            ...
            e.preventDefault();
            var ajaxData = {
                action: 'qodef_membership_register_user',
                security: $(this).find('#qodef-register-security').val(),
                register_data: $(this).serialize()
            };
            var message = $(this).find('#register-form-submitted').val();
	        var formSubmitParams = {status:"sent", message:message};
	        qodefRenderAjaxResponseMessage(formSubmitParams);
            $.ajax({
                type: 'POST',
    ...
    function qodefUserLostPassword() {
        ...
        lostPassForm.submit(function (e) {
            e.preventDefault();
            var data = {
                action: 'qodef_membership_user_lost_password',
                user_login: lostPassForm.find('#user_reset_password_login').val()
            };
	        var message = $(this).find('#password-form-submitted').val();
            var formSubmitParams = {status:"sent", message:message};
	        qodefRenderAjaxResponseMessage(formSubmitParams);
            $.ajax({
                type: 'POST',
    ...
    function qodefRenderAjaxResponseMessage(response) {
	    ...
	    if (response.status === 'sent') {
		    messageClass = 'qodef-membership-message-sent';
	    }
    ...
    function qodefGetFacebookUserData() {
        ...
        FB.api('/me', 'GET', {'fields': 'id, name, email, link, picture'}, function (response) {
            ...
            var data = {
                action: 'qodef_membership_check_facebook_user',
                response: response
            };
            $.ajax({
                type: 'POST',
    ...
    function qodefSignInCallback() {
        ...
        if (signedIn) {
            var currentUser = window.auth2.currentUser.get(),
                ...
                data = {
                    action: 'qodef_membership_check_google_user',
                    response: userData
                };
            $.ajax({
                type: 'POST',
...
(function($) {
    ...
    function qodefMembershipWhishlistAdding(item, itemID){
        ...
        var ajaxData = {
            action: 'qodef_membership_add_item_to_favorites',
            item_id : itemID
        };
    ...
    function qodefMembershipAddToWishlistTriggerEvent() {
        $( document.body ).on( 'qodef_membership_favorites_trigger', function() {
            qodefMembershipAddToWishlist();
        });
    }
