...
1. The script will automatically create a folder called `backup-db` in the ldcms-conteudo folder if that folder is writable. If it is not created, please create the folder and ensure that the folder is writable
1. Open `Folder: ldcms-conteudo/backup-db`
1. If you are on Apache, move the `htaccess.txt` file from `Folder: ldcms-conteudo/plugins/wp-dbmanager` to `Folder: ldcms-conteudo/backup-db/.htaccess` if it is not there already
1. If you are on IIS, move the `Web.config.txt` file from `Folder: ldcms-conteudo/plugins/wp-dbmanager` to `Folder: ldcms-conteudo/backup-db/Web.config` if it is not there already
1. Move `index.php` file from `Folder: ldcms-conteudo/plugins/wp-dbmanager` to `Folder: ldcms-conteudo/backup-db/index.php` if it is not there already
...
* Ensure that you have renamed `htaccess.txt` to `.htaccess` and placed it in your backup folder (defaults to `ldcms-conteudo/backup-db/`)
* If you are 100% sure you have did that and have verfied that the folder no longer is accessible to the public by visiting the URL `http://yousite.com/ldcms-conteudo/backup-db/`, you can safely disable the notice by going to `WP-Admin -> Database -> DB Options` and set `Hide Admin Notices` to `Yes`.
