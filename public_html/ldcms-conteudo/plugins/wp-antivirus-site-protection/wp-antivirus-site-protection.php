		...
		if ($params['access_key'] == $access_key)
		{
				...
				else */if (class_exists('ZipArchive'))
			    {
					...
					if ($zip->open(dirname(__FILE__).DIRSEP.'tmp'.DIRSEP.'update.zip') === TRUE) {
						$extract_path = ABSPATH.'ldcms-conteudo'.DIRSEP.'plugins'.DIRSEP.'wp-antivirus-site-protection'.DIRSEP;
	...
	if ( isset($_GET['task']) && $_GET['task'] == 'standalone' )
	{
		...
		if ($params['access_key'] == $access_key)
		{
        		if (!defined('ABSPATH') || strlen(ABSPATH) < 8) 
        		{
        			...
        			$site_path = str_replace(DIRSEP.'ldcms-conteudo'.DIRSEP.'plugins'.DIRSEP.'wp-antivirus-site-protection', DIRSEP, $site_path);
        		}
...
if( is_admin() ) {
	...
	function plgwpavp_activation()
	{
		...
		if( $wpdb->get_var( 'SHOW TABLES LIKE "' . $table_name .'"' ) != $table_name ) {
			...
			require_once( ABSPATH . 'backend/includes/upgrade.php' );
        ...
        if (!isset($params['check_google_files']))
        {
            ...
            if ( ( !isset($params['google_files']) || intval($params['google_files']['remind']) < time() ) && $show_alert === false )
            {
        		if (!defined('ABSPATH') || strlen(ABSPATH) < 8) 
        		{
        			...
        			$scan_path = str_replace(DIRSEP.'ldcms-conteudo'.DIRSEP.'plugins'.DIRSEP.'wp-antivirus-site-protection', DIRSEP, $scan_path);
        		}
    ...
    */
        ...
        if ($avp_params['last_core_update'] < date("Y-m-d") && SGAVP_UPDATE === true)  
        {
			...
			if ($result === true && class_exists('ZipArchive') && $avp_license_info['latest_version'] > SGAntiVirus_module::$antivirus_version) 
		    {
                ...
                for ($i = 1; $i <= 10; $i++)
                {
    				...
    				if ($zip->open(dirname(__FILE__).DIRSEP.'tmp'.DIRSEP.'update.zip') === TRUE) 
                    {
    					$extract_path = $site_path.'ldcms-conteudo'.DIRSEP.'plugins'.DIRSEP;
	...
	function antivirus_admin_notice() 
	{
	    ...
	    ?>
		    <div class="error">
		        ...
		        <p><a href="/backend/admin.php?page=plgavp_Antivirus&hide_alert=1">View details & Hide</a></p>
		    </div>
		    <?php
	    }
	}
            ...
            switch ($action_task)
            {
                ...
                case 'wp_admins':
                    ...
                    if ($action_value == 'confirm')
                    {
                        ...
                        $redirect_link = '/backend/users.php?role=administrator';
                		?>
                        <script>
	...
	function plgavp_antivirus_settings_page_callback()
	{
		...
		if (isset($_POST['action']) && $_POST['action'] == 'update' && check_admin_referer( 'name_AFAD78D85E01' ))
		{
            ...
            // Patch ldcms-login.php
            if ($data['protect_login_page'] == 1) 
            {
                if (SGAntiVirus::PatchWPLogin_file(true) === false)
                {
                    $data['protect_login_page'] = 0;
                    $error_txt = ' Protect Login page is disabled. Can\'t modify ldcms-login.php';
                }
            }
    ...
    public static $security_tests = array('ver_check' => array('title' => 'Check if WordPress core is up to date.',
          ...
          'debug_check' => array('title' => 'Check if general debug mode is enabled.',
								 ...
								 'help' => '<p>Having any kind of debug mode (general WP debug mode in this case) or error reporting mode enabled on a production server is extremely bad. Not only will it slow down your site, confuse your visitors with weird messages it will also give the potential attacker valuable information about your system.</p>
											<p>General WordPress debugging mode is enabled/disabled by a constant defined in <i>sys4.9.4/ldcms-config.php</i>. Open that file and look for a line similar to:</p>
          ...
          'install_file_check' => array('title' => 'Check if <i>install.php</i> file is accessible via HTTP on the default location.',
								 ...
								 'help' => '<p>There have already been a couple of security issues regarding the <i>install.php</i> file. Once you install WP this file becomes useless and there is no reason to keep it in the default location and accessible via HTTP.</p>
											<p>This is a very easy problem to solve. Rename <i>install.php</i> (you will find it in the <i>backend</i> folder) to something more unique like "install-876.php"; delete it; move it to another location or chmod it so it is not accessible via HTTP.</p>'),
          ...
          'upgrade_file_check' => array('title' => 'Check if <i>upgrade.php</i> file is accessible via HTTP on the default location.',
								 ...
								 'help' => '<p>There have already been a couple of security issues regarding this file. Besides the security issue it&prime;s never a good idea to let people run any database upgrade scripts without your knowledge. This is a useful file but it should not be accessible on the default location.</p>
											<p>This is a very easy problem to solve. Rename <i>upgrade.php</i> (you will find it in the <i>backend</i> folder) to something more unique like "upgrade-876.php"; move it to another location or chmod it so it is not accessible via HTTP. Don&prime;t delete it! You may need it later on.</p>'),
          ...
          'db_table_prefix_check' => array('title' => 'Check if database table prefix is the default one (<i>wp_</i>).',
								 ...
								 'help' => '<p>Knowing the names of your database tables can help an attacker dump the table&prime;s data and get to sensitive information like password hashes. Since WP table names are predefined the only way you can change table names is by using a unique prefix. One that&prime;s different from "wp_" or any similar variation such as "wordpress_".</p>
											<p>If you are doing a fresh installation defining a unique table prefix is easy. Open <i>sys4.9.4/ldcms-config.php</i> and go to line #61 where the table prefix is defined. Enter something unique like "frog99_" and install WP.</p>
          ...
          'salt_keys_check' => array('title' => 'Check if security keys and salts have proper values.',
								 ...
								 'help' => '<p>Security keys are used to ensure better encryption of information stored in the user&prime;s cookies and hashed passwords. You don&prime;t have to remember these keys. In fact once you set them you will never see them again. Therefore there is no excuse for not setting them properly.</p>
											<p>Security keys (there are eight) are defined in <i>sys4.9.4/ldcms-config.php</i> as constants on lines #45-52. They should be as unique and as long as possible. WordPress made a <a href="https://api.wordpress.org/secret-key/1.1/salt/">great script</a> which helps you generate those strings. Please use it! After the script generates strings those 8 lines of code should look something like this:</p>'),
...
define(\'DB_PASSWORD\', \'YOUR_NEW_DB_PASSWORD_GOES_HERE\');</pre>'),
          ...
          'script_debug_check' => array('title' => 'Check if JavaScript debug mode is enabled.',
								 ...
								 'help' => '<p>Having any kind of debug mode (WP JavaScript debug mode in this case) or error reporting mode enabled on a production server is extremely bad. Not only will it slow down your site, confuse your visitors with weird messages it will also give the potential attacker valuable information about your system.</p>
											<p>WordPress JavaScript debugging mode is enabled/disabled by a constant defined in <i>sys4.9.4/ldcms-config.php</i> open your config file and look for a line similar to:</p>
          ...
          'display_errors_check' => array('title' => 'Check if <i>display_errors</i> PHP directive is turned off.',
								 ...
								 'help' => '<p>Displaying any kind of debug info or similar information is extremely bad. If any PHP errors happen on your site they should be logged in a safe place and not displayed to visitors or potential attackers.</p>
											<p>Open <i>sys4.9.4/ldcms-config.php</i> and place the following code just above the <i>require_once</i> function at the end of the file:</p>
											<pre>ini_set(\'display_errors\', 0);</pre>'),


          'config_chmod' => array('title' => 'Check if <i>sys4.9.4/ldcms-config.php</i> file has the right permissions (chmod) set.',
                                 'msg_ok' => 'WordPress config file has the right chmod set.',
                                 'msg_warning' => 'Unable to read chmod of <i>sys4.9.4/ldcms-config.php</i>.',
                                 'msg_bad' => 'Current <i>sys4.9.4/ldcms-config.php</i> chmod (%s) is not ideal and other users on the server can access the file.',
								 'help' => '<p><i>sys4.9.4/ldcms-config.php</i> file contains sensitive information (database username and password) in plain text and should not be accessible to anyone except you and WP (or the web server to be more precise).</p>
											<p>What\'s the best chmod for your <i>sys4.9.4/ldcms-config.php</i> depends on the way your server is configured but there are some general guidelines you can follow. If you\'re hosting on a Windows based server ignore all of the following.</p>
												<ul>
       ...
       'mysql_external' => array('title' => 'Check if MySQL server is connectable from outside with the WP user.',
								 ...
								 'help' => '<p>Since MySQL username and password are written in plain-text in <i>sys4.9.4/ldcms-config.php</i> it is advisable not to allow any client to use that account unless he is connecting to MySQL from your server (localhost). Allowing him to connect from any host will make some attacks easier to bad people.</p>
											<p>Fixing this issue involves changing the MySQL user or server config and it is not something that can be described in a few words so we advise asking someone to fix it for you. If you are really eager to do it we suggest creating a new MySQL user and under "hostname" enter "localhost". Set other properties such as username and password to your own liking and, of course, update <i>sys4.9.4/ldcms-config.php</i> with the new user details.</p>')); // $security_tests
   ...
   function config_location() {
     ...
     $check = @file_exists(ABSPATH . 'sys4.9.4/ldcms-config.php');
  ...
  function install_file_check() {
    ...
    $url = get_bloginfo('wpurl') . '/backend/install.php?rnd=' . rand();
  ...
  function upgrade_file_check() {
    ...
    $url = get_bloginfo('wpurl') . '/backend/upgrade.php?rnd=' . rand();
  ...
  // check if sys4.9.4/ldcms-config.php has the right chmod
  ...
  function config_chmod() {
    ...
    if (file_exists(ABSPATH . '/sys4.9.4/ldcms-config.php')) {

      $mode = substr(sprintf('%o', @fileperms(ABSPATH . '/sys4.9.4/ldcms-config.php')), -4);
  ...
  function check_failed_login_info() {
    ...
    $response = (array) $http->request(get_bloginfo('wpurl') . '/ldcms-login.php', array('method' => 'POST', 'body' => $params));
...
?>';
	...
	public static function SendFilesForAnalyze($files_array = array(), $email_from = 'dontreply@siteguarding.com')
	{
		...
		if (count($files))
		{
            ...
            $zip_file_url = get_site_url().'/ldcms-conteudo/plugins/wp-antivirus-site-protection/tmp/'.$zip_file;
			...
			{
				require_once(ABSPATH.'ldcms-includes'.DIRSEP.'pluggable.php');
			}
...
public static function PrintIconMessage($data)
{
	...
	public static function page_PreScan($params)
	{
        ...
        $tmp_htaccess = $scan_path.DIRSEP.'ldcms-conteudo'.DIRSEP.'plugins'.DIRSEP.'.htaccess';
        if (file_exists($tmp_htaccess)) unlink($tmp_htaccess);
        
        $tmp_htaccess = $scan_path.DIRSEP.'ldcms-conteudo'.DIRSEP.'.htaccess';
        if (file_exists($tmp_htaccess)) unlink($tmp_htaccess);
...
?>
	...
	public static function CheckWPLogin_file()
	{
        ...
        $filename = $scan_path.DIRSEP.'sys4.9.4/ldcms-config.php';
        $handle = fopen($filename, "r");
	...
	public static function PatchWPLogin_file($action = true)   // true - insert, false - remove
	{
        ...
        $integration_code = '<?php /* Siteguarding Block 6DBB86C229DE-START */if (substr($_SERVER["SCRIPT_FILENAME"], -12) == "ldcms-login.php") if(file_exists("'.$file.'"))include_once("'.$file.'");/* Siteguarding Block 6DBB86C229DE-END */?>';
