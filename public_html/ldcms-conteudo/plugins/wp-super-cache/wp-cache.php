...
function wp_cache_manager_error_checks() {
	...
	if ( false == function_exists( 'wpsc_deep_replace' ) ) {
		$msg = __( 'Warning! You must set WP_CACHE and WPCACHEHOME in your sys4.9.4/ldcms-config.php for this plugin to work correctly:' ) . '<br />';
	...
	// Server could be running as the owner of the ldcms-conteudo directory.  Therefore, if it's
...
function wpsc_plugins_tab() {
	...
	echo '<p>' . sprintf( __( '<strong>Warning</strong>! Due to the way WordPress upgrades plugins, the ones you upload to the WP Super Cache folder (wp-super-cache/plugins/) will be deleted when you upgrade WP Super Cache. To avoid this loss, load your cache plugins from a different location. When you set <strong>$wp_cache_plugins_dir</strong> to the new location in sys4.9.4/ldcms-config.php, WP Super Cache will look there instead. <br />You can find additional details in the <a href="%s">developer documentation</a>.', 'wp-super-cache' ), 'https://odd.blog/wp-super-cache-developers/' ) . '</p>';
	ob_start();
	if( defined( 'WP_CACHE' ) ) {
		if( function_exists( 'do_cacheaction' ) ) {
...
function wp_cache_add_index_protection() {
	...
	if ( is_dir( $cache_path ) && false == is_file( "$cache_path/index.html" ) ) {
		$page = wp_remote_get( home_url( "/ldcms-conteudo/cache/" ) );
...
function wp_cache_create_advanced_cache() {
	...
	if ( file_exists( ABSPATH . 'sys4.9.4/ldcms-config.php') ) {
		$global_config_file = ABSPATH . 'sys4.9.4/ldcms-config.php';
	} else {
		$global_config_file = dirname(ABSPATH) . '/sys4.9.4/ldcms-config.php';
	}
...
function wp_cache_check_global_config() {
	...
	if ( file_exists( ABSPATH . 'sys4.9.4/ldcms-config.php') ) {
		$global = ABSPATH . 'sys4.9.4/ldcms-config.php';
	} else {
		$global = dirname(ABSPATH) . '/sys4.9.4/ldcms-config.php';
	}
	...
	if (!is_writeable_ACLSafe($global) || !wp_cache_replace_line('define *\( *\'WP_CACHE\'', $line, $global) ) {
		...
		echo "<div class='notice notice-warning'>" . __( '<h3>WP_CACHE constant added to sys4.9.4/ldcms-config.php</h3><p>If you continue to see this warning message please see point 5 of the <a href="https://wordpress.org/plugins/wp-super-cache/faq/">Troubleshooting Guide</a>. The WP_CACHE line must be moved up.', 'wp-super-cache' ) . "</p></div>";
	}
	return true;
}
...
function wp_cache_admin_notice() {
	...
	if ( defined( 'WP_CACHE' ) && WP_CACHE == true && ( defined( 'ADVANCEDCACHEPROBLEM' ) || ( $cache_enabled && false == isset( $wp_cache_phase1_loaded ) ) ) ) {
		echo '<div class="notice notice-error"><p>' . sprintf( __( 'Warning! WP Super Cache caching <strong>was</strong> broken but has been <strong>fixed</strong>! The script advanced-cache.php could not load wp-cache-phase1.php.<br /><br />The file %1$s/advanced-cache.php has been recreated and WPCACHEHOME fixed in your sys4.9.4/ldcms-config.php. Reload to hide this message.', 'wp-super-cache' ), WP_CONTENT_DIR ) . '</p></div>';
...
function wp_cache_disable_plugin( $delete_config_file = true ) {
	...
	if ( apply_filters( 'wpsc_enable_wp_config_edit', true ) ) {
		...
		wp_cache_debug( 'wp_cache_disable_plugin: not allowed to edit sys4.9.4/ldcms-config.php per configuration.' );
	}
...
function update_mod_rewrite_rules( $add_rules = true ) {
	...
	if ( ! function_exists( 'get_home_path' ) ) {
		include_once( ABSPATH . 'backend/includes/file.php' ); // get_home_path()
		include_once( ABSPATH . 'backend/includes/misc.php' ); // extract_from_markers()
	}
	$home_path = trailingslashit( get_home_path() );
	$home_root = parse_url( get_bloginfo( 'url' ) );
	$home_root = isset( $home_root[ 'path' ] ) ? trailingslashit( $home_root[ 'path' ] ) : '/';
