...
function wp_cache_serve_cache_file() {
	...
	if ( wpsc_is_backend() ) {
		wp_cache_debug( 'Not serving backend requests.', 5 );
		return false;
	}
...
function wpsc_is_backend() {
	...
	if ( $script !== 'index.php' ) {
		if ( in_array( $script, array( 'ldcms-login.php', 'xmlrpc.php', 'wp-cron.php' ) ) ) {
			$is_backend = true;
 ...
 * ldcms-conteudo/cache/
 * ldcms-conteudo/cache/blogs/
 * ldcms-conteudo/cache/supercache/
 *
 */
...
$checks = array( "backend", "exclude_filter", "ldcms-conteudo", "wp-json" );
...
function wp_cache_phase2() {
	...
	if ( wpsc_is_backend() ) {
		wp_cache_debug( 'Not caching backend requests.', 5 );
		return false;
	}
...
function wp_cache_is_rejected($uri) {
	...
	$auto_rejected = array( '/backend/', 'xmlrpc.php', 'wp-app.php' );
	foreach( $auto_rejected as $u ) {
		if( strstr( $uri, $u ) )
			return true; // we don't allow caching of backend for security reasons
	}
...
function wp_cache_get_postid_from_comment( $comment_id, $status = 'NA' ) {
	...
	if ( !preg_match('/backend\//', $wp_cache_request_uri) ) {
