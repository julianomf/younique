...
The cache directory, usually ldcms-conteudo/cache/ is only for temporary files. Do not ever put important files or symlinks to important files or directories in that directory. They will be deleted if the plugin has write access to them.
...
1.  Is ldcms-conteudo writable by the web server?
2.  Is there a ldcms-conteudo/wp-cache-config.php ? If not, copy the file wp-super-cache/wp-cache-config-sample.php to ldcms-conteudo/wp-cache-config.php and make sure WPCACHEHOME points at the right place.
3.  Is there a ldcms-conteudo/advanced-cache.php ? If not, then you must copy wp-super-cache/advanced-cache.php into ldcms-conteudo/. You must edit the file and change the path so it points at the wp-super-cache folder.
4.  If pages are not cached at all, remove ldcms-conteudo/advanced-cache.php and recreate it, following the advice above.
5.  Make sure the following line is in sys4.9.4/ldcms-config.php and it is ABOVE the "require_once(ABSPATH.'wp-settings.php');" line:

    `define( 'WP_CACHE', true );`
6.  Try the Settings->WP Super Cache page again and enable cache.
7.  Look in ldcms-conteudo/cache/supercache/. Are there directories and files there?
...
12. Private Server users at Dreamhost should edit ldcms-conteudo/wp-cache-config.php and set the cache dir to "/tmp/" if they are getting errors about increasing CPU usage. See this [discussion](https://wordpress.org/support/topic/145895?replies=42) for more.
13. File locking errors such as "failed to acquire key 0x152b: Permission denied in..." or "Page not cached by WP Super Cache. Could not get mutex lock." are a sign that you may have to use file locking. Edit ldcms-conteudo/wp-cache-config.php and uncomment "$use_flock = true" or set $sem_id to a different value. You can also disable file locking from the Admin screen as a last resort.
...
18. The error message, "WP Super Cache is installed but broken. The path to wp-cache-phase1.php in ldcms-conteudo/advanced-cache.php must be fixed!" appears at the end of every page. Open the file ldcms-conteudo/advanced-cache.php in your favourite editor. Is the path to wp-cache-phase1.php correct? This file will normally be in ldcms-conteudo/plugins/wp-super-cache/. If it is not correct the caching engine will not load.
...
20. If supercache cache files are generated but not served, check the permissions on all your ldcms-conteudo/cache/supercache folders (and each of ldcms-conteudo cache and supercache folders) and ldcms-conteudo/cache/.htaccess. If your PHP runs as a different user to Apache and permissions are strict Apache may not be able to read the PHP generated cache files. To fix you must add the following line to your sys4.9.4/ldcms-config.php (Add it above the WP_CACHE define.) Then clear your cache.
...
If that doesn't work, add this line to your sys4.9.4/ldcms-config.php:
...
24. If your blog refuses to load make sure your sys4.9.4/ldcms-config.php is correct. Are you missing an opening or closing PHP tag?
...
18. The error message, "WP Super Cache is installed but broken. The constant WPCACHEHOME must be set in the file sys4.9.4/ldcms-config.php and point at the WP Super Cache plugin directory." appears at the end of every page. You can delete ldcms-conteudo/advanced-cache.php and reload the plugin settings page or edit sys4.9.4/ldcms-config.php and look for WPCACHEHOME and make sure it points at the wp-super-cache folder. This will normally be ldcms-conteudo/plugins/wp-super-cache/ but you'll likely need the full path to that file (so it's easier to let the settings page fix it). If it is not correct the caching engine will not load.
...
3. Remove the WP_CACHE define from sys4.9.4/ldcms-config.php. It looks like `define( 'WP_CACHE', true );`
4. Remove the Super Cache mod_rewrite rules from your .htaccess file.
5. Remove the files ldcms-conteudo/advanced-cache.php and ldcms-conteudo/wp-cache-config.php
6. Remove the directory ldcms-conteudo/cache/
7. Remove the directory wp-super-cache from your plugins directory.

### If all else fails and your site is broken ###
1. Remove the WP_CACHE define from sys4.9.4/ldcms-config.php. It looks like `define( 'WP_CACHE', true );`
...
4. Optionally delete advanced-cache.php, wp-cache-config.php and the cache folder in ldcms-conteudo/.
...
If you only want to use the WP-Cache engine then edit your sys4.9.4/ldcms-config.php or create an mu-plugin that sets the constant 'DISABLE_SUPERCACHE' to 1.

### WP-Cache vs Supercache files ###
All cache files are stored in ldcms-conteudo/cache/supercache/HOSTNAME/ where HOSTNANE is your domain name. The files are stored in directories matching your site's permalink structure. Supercache files are index.html or some variant of that, depending on what type of visitor hit the blog. Other files are named wp-cache-XXXXXXXXXXXXXXXXX.php. Associated meta filesnames start with "meta". Those files contain information about the cached file. These files are generated by the "WPCache caching" engine in the plugin.
...
### The plugin complains that ldcms-conteudo is writable! htdocs is writable! ###
...
### How do I delete the WP_CACHE define from sys4.9.4/ldcms-config.php? ###
Load your desktop ftp client and connect to your site. Navigate to the root (or the directory below it) of your site where you'll find sys4.9.4/ldcms-config.php. Download that file and edit it in a text editor. Delete the line `define( 'WP_CACHE', true );` and save the file. Now upload it, overwriting the sys4.9.4/ldcms-config.php on your server.
...
* Added the "wpsc_enable_wp_config_edit" filter to disable editing the sys4.9.4/ldcms-config.php (#392)
...
* Delete WPCACHEHOME and WP_CACHE from sys4.9.4/ldcms-config.php when plugin deactivated.
...
* Update advanced-cache.php and sys4.9.4/ldcms-config.php if wp-cache-phase1.php doesn't load, usually happening after migrating to a new hosting service.
* Misc bugfixes.
