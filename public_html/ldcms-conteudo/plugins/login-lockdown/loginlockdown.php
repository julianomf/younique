...
* - fixed activation error caused by customizing the location of the ldcms-conteudo folder
...
function ll_credit_link(){
	...
	if ( $showcreditlink != "shownofollow" && ($thispage == $homepage || $thispage == $homepage . "/" || substr($_SERVER["REQUEST_URI"], strlen($_SERVER["REQUEST_URI"]) - 12) == "ldcms-login.php") ) {
		$relnofollow = "";
	}
...
if ( isset($loginlockdown_db_version) ) {
	...
	if(!defined('WP_PLUGIN_DIR')){
		define('WP_PLUGIN_DIR', ABSPATH . 'ldcms-conteudo/plugins');
	}
	...
	function ll_wp_authenticate_username_password($user, $username, $password) {
		...
		if ( !$userdata ) {
			return new WP_Error('invalid_username', sprintf(__('<strong>ERROR</strong>: Invalid username. <a href="%s" title="Password Lost and Found">Lost your password</a>?'), site_url('ldcms-login.php?action=lostpassword', 'login')));
		}
		...
		if ( !wp_check_password($password, $userdata->user_pass, $userdata->ID) ) {
			return new WP_Error('incorrect_password', sprintf(__('<strong>ERROR</strong>: Incorrect password. <a href="%s" title="Password Lost and Found">Lost your password</a>?'), site_url('ldcms-login.php?action=lostpassword', 'login')));
		}
	...
	function wp_authenticate($username, $password) {
		...
		if (is_wp_error($user) && !in_array($user->get_error_code(), $ignore_codes) ) {
			...
			if ( 'yes' == $loginlockdownOptions['mask_login_errors'] ) {
				return new WP_Error('authentication_failed', sprintf(__('<strong>ERROR</strong>: Invalid username or incorrect password. <a href="%s" title="Password Lost and Found">Lost your password</a>?'), site_url('ldcms-login.php?action=lostpassword', 'login')));
			} else {
