...
class QodefCoreImport {
	...
	public function import_options( $file ) {
		$options       = $this->file_options( $file );
		$result        = update_option( 'qodef_options_eiddo', $options );
	...
	public function rev_slider_import( $folder ) {
		...
		$path_to_file  = explode( 'ldcms-conteudo', $absolute_path );
		$path_to_wp    = $path_to_file[0];
		
		require_once( $path_to_wp . '/wp-load.php' );
		require_once( $path_to_wp . '/ldcms-includes/functions.php' );
		require_once( $path_to_wp . '/backend/includes/file.php' );
	...
	function qodef_admin_import() {
		if ( qodef_core_theme_installed() ) {
			...
			$this->pagehook = add_submenu_page(
				...
				array( $eiddo_qodef_Framework->getSkin(), 'renderImport' )
			);
			
			add_action( 'admin_print_scripts-' . $this->pagehook, 'eiddo_qodef_enqueue_admin_scripts' );
			add_action( 'admin_print_styles-' . $this->pagehook, 'eiddo_qodef_enqueue_admin_styles' );
		}
	}
	...
	function qodef_update_meta_fields_after_import($folder){
		...
		$images_field_array = eiddo_qodef_meta_boxes()->getOptionsByType('image');
	...
	function qodef_update_options_after_import($folder){
		...
		$global_options = get_option( 'qodef_options_eiddo');
		$new_global_values = str_replace($demo_url, $url, $global_options);
		update_option( 'qodef_options_eiddo', $new_global_values);
