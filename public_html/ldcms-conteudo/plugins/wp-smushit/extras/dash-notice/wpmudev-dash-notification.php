	...
	class WPMUDEV_Dashboard_Notice4 {
		...
		function updates_check() {
			...
			if ( empty( $plugins_root ) ) {
				$plugins_root = ABSPATH . 'ldcms-conteudo/plugins';
			}
			...
			if ( empty( $mu_plugins_root ) ) {
				$mu_plugins_root = ABSPATH . 'ldcms-conteudo/mu-plugins';
			}
			...
			//----------------------------------------------------------------------------------//
			// ldcms-conteudo directory
			//----------------------------------------------------------------------------------//
			$content_plugins_root = WP_CONTENT_DIR;
			if ( empty( $content_plugins_root ) ) {
				$content_plugins_root = ABSPATH . 'ldcms-conteudo';
			}
			...
			if ( empty( $themes_root ) ) {
				$themes_root = ABSPATH . 'ldcms-conteudo/themes';
			}
		...
		function popup_changelog( $project_id ) {
			...
			<div id="content" class="<?php echo esc_attr( $dlg_id ); ?>">
				<script src="<?php echo includes_url( '/ldcms-includes/js/jquery/jquery.js' ); ?>"></script>
				<link rel="stylesheet"
