...
function _mw_adminimize_remove_dashboard() {
	...
	if ( $disabled_menu_all !== '' || $disabled_submenu_all !== '' ) {
		...
		if ( $redirect ) {
			...
			$_mw_adminimize_db_redirect_admin_url = get_option( 'siteurl' ) . '/backend/';
			...
			if ( preg_match( '#backend/?(index.php)?$#', $_SERVER[ 'REQUEST_URI' ] ) ) {
	...
	<div id="small_user_info">
		<p>
			<a href="<?php echo wp_nonce_url(
				site_url( 'ldcms-login.php?action=logout' ),
				'log-out'
			) ?>"
