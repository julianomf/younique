	...
	public static function putAdminBarMenus () {
		...
		<script>	
			jQuery(document).ready(function() {			
				...
				if (jQuery('#backend-bar-revslider-default').length>0 && jQuery('.rev_slider_wrapper').length>0) {
					var aliases = new Array();
					jQuery('.rev_slider_wrapper').each(function() {
						aliases.push(jQuery(this).data('alias'));
					});								
					if(aliases.length>0)
						jQuery('#backend-bar-revslider-default li').each(function() {
							var li = jQuery(this),
					...
					jQuery('#backend-bar-revslider').remove();
				}
			});
		</script>
		<?php 	
