...
class LoginWithAjax {
	...
	public static function init(){
		...
		self::$url_login = self::template_link(site_url('ldcms-login.php', 'login_post'));
		self::$url_register = self::template_link(self::getRegisterLink());
		self::$url_remember = self::template_link(site_url('ldcms-login.php?action=lostpassword', 'login_post'));
	...
	public static function remember(){
		$return = array(); //What we send back
		//if we're not on ldcms-login.php, we need to load it since retrieve_password() is located there
	...
	public static function getRegisterLink(){
	    ...
	    if ( function_exists('bp_get_signup_page') && (empty($_REQUEST['action']) || ($_REQUEST['action'] != 'deactivate' && $_REQUEST['action'] != 'deactivate-selected')) ) { //Buddypress
	    	...
	    	$register_link = site_url('ldcms-login.php?action=register', 'login');
	    }
	...
	public static function new_user_notification($user_login, $login_link, $user_email, $blogname){
		//Copied out of /ldcms-includes/pluggable.php
	 ...
	 * /ldcms-conteudo/themes/your-child-theme/plugins/login-with-ajax/...
	 * /ldcms-conteudo/themes/your-parent-theme/plugins/login-with-ajax/...
