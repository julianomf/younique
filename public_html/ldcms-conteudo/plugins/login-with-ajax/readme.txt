...
1. Upload this plugin to the `/ldcms-conteudo/plugins/` directory and unzip it, or simply upload the zip file within your wordpress installation.
...
* profile_link - (1 or 0)
 * If value is 1 (default), a profile link to backend appears.
* registration - (1 or 0)
...
* When looking for files/templates there is an order of precedence - active child theme (if applicable), active parent themes, and finally the plugin folder:
  * `ldcms-conteudo/themes/your-theme/plugins/login-with-ajax/`
  * `ldcms-conteudo/themes/parent-theme/plugins/login-with-ajax/`
  * `ldcms-conteudo/plugins/login-with-ajax/widget/`
 ...
 * `ldcms-conteudo/plugins/login-with-ajax/widget/login-with-ajax.js`
 * `ldcms-conteudo/plugins/login-with-ajax/widget/widget.css`
...
If you wanted to change some text on the default theme, you could simply copy `ldcms-conteudo/plugins/login-with-ajax/widget/default` to `ldcms-conteudo/themes/yourtheme/plugins/login-with-ajax/default` and edit the files as needed.

If you need to change the CSS file, copy the file `ldcms-conteudo/plugins/login-with-ajax/widget/widget.css` over to `ldcms-conteudo/themes/yourtheme/plugins/login-with-ajax/widget.css` and edit accordingly.
...
Firstly, you should make sure that you can register via the normal backend login, if something goes wrong there the problem is not login with ajax. Please note that currently there is no AJAX registration with BuddyPress due to it rewriting the login area (this will be resolved soon).
...
* removed inclusion of ldcms-includes/registration.php during regsitration (not needed since WP 3.1)
