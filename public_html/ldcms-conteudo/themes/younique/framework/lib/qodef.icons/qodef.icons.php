...
/*
  Class: EiddoQodefIconCollections
...
class EiddoQodefIconCollections {
    ...
    private function initIconCollections() {
        $this->addIconCollection('font_awesome', new EiddoQodefIconsFontAwesome("Font Awesome", "fa_icon"));
        $this->addIconCollection('font_elegant', new EiddoQodefIconsFontElegant("Font Elegant", "fe_icon"));
        $this->addIconCollection('ion_icons', new EiddoQodefIonIcons("Ion Icons", "ion_icon"));
        $this->addIconCollection('linea_icons', new EiddoQodefLineaIcons('Linea Icons', 'linea_icon'));
        $this->addIconCollection('linear_icons', new EiddoQodefLinearIcons('Linear Icons', 'linear_icon'));
        $this->addIconCollection('simple_line_icons', new EiddoQodefSimpleLineIcons('Simple Line Icons', 'simple_line_icon'));
        $this->addIconCollection('dripicons', new EiddoQodefDripicons('Dripicons', 'dripicon'));
    }
    ...
    public function getIconsMetaBoxOrOption($attributes) {
	    ...
	    $options = array(
		    'font_awesome'      => esc_html__( 'Font Awesome', 'eiddo' ),
		    'font_elegant'      => esc_html__( 'Font Elegant', 'eiddo' ),
		    'ion_icons'         => esc_html__( 'Ion Icons', 'eiddo' ),
		    'simple_line_icons' => esc_html__( 'Simple Line Icons', 'eiddo' )
	    );
	    ...
	    if ( $scope == 'regular' ) {
		    $options = array(
			    ...
			    'font_elegant'      => esc_html__( 'Font Elegant', 'eiddo' ),
			    'ion_icons'         => esc_html__( 'Ion Icons', 'eiddo' ),
			    'linea_icons'       => esc_html__( 'Linea Icons', 'eiddo' ),
			    'linear_icons'      => esc_html__( 'Linear Icons', 'eiddo' ),
			    'simple_line_icons' => esc_html__( 'Simple Line Icons', 'eiddo' ),
			    'dripicons'         => esc_html__( 'Dripicons', 'eiddo' )
		    );
	    }
	    ...
	    foreach ( $icon_collections as $collection_key => $collection_object ) {
		    ...
		    $qodef_icon_pack_container = eiddo_qodef_add_admin_container(
			    array(
				    'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'eiddo' ),
	...
	public function getIconWidgetParamsArray() {
		...
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'eiddo' ),
			'options' => array(
				'font_awesome'      => esc_html__( 'Font Awesome', 'eiddo' ),
				'font_elegant'      => esc_html__( 'Font Elegant', 'eiddo' ),
				'ion_icons'         => esc_html__( 'Ion Icons', 'eiddo' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'eiddo' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			...
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'eiddo' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'eiddo' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'eiddo' ), $n ),
	...
	public function getIconCollectionIcons( iEiddoQodefIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'eiddo' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'eiddo' ) ] = '';
...
if ( ! function_exists( 'eiddo_qodef_activate_theme_icons' ) ) {
	function eiddo_qodef_activate_theme_icons() {
		global $eiddo_qodef_IconCollections;
		$eiddo_qodef_IconCollections = EiddoQodefIconCollections::get_instance();
	}
	
	add_action( 'after_setup_theme', 'eiddo_qodef_activate_theme_icons' );
}