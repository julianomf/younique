...
if ( ! function_exists( 'eiddo_qodef_header_skin_class' ) ) {
	/**
	 * Function that adds header style class to body tag
	 */
	function eiddo_qodef_header_skin_class( $classes ) {
		$header_style     = eiddo_qodef_get_meta_field_intersect( 'header_style', eiddo_qodef_get_page_id() );
		$header_style_404 = eiddo_qodef_options()->getOptionValue( '404_header_style' );
	...
	add_filter( 'body_class', 'eiddo_qodef_header_skin_class' );
}

if ( ! function_exists( 'eiddo_qodef_sticky_header_behaviour_class' ) ) {
	/**
	 * Function that adds header behavior class to body tag
	 */
	function eiddo_qodef_sticky_header_behaviour_class( $classes ) {
		$header_behavior = eiddo_qodef_get_meta_field_intersect( 'header_behaviour', eiddo_qodef_get_page_id() );
	...
	add_filter( 'body_class', 'eiddo_qodef_sticky_header_behaviour_class' );
}

if ( ! function_exists( 'eiddo_qodef_menu_dropdown_appearance' ) ) {
	/**
	 ...
	 */
	function eiddo_qodef_menu_dropdown_appearance( $classes ) {
		$dropdown_menu_appearance = eiddo_qodef_options()->getOptionValue( 'menu_dropdown_appearance' );
	...
	add_filter( 'body_class', 'eiddo_qodef_menu_dropdown_appearance' );
}

if ( ! function_exists( 'eiddo_qodef_header_class' ) ) {
	/**
	 ...
	 */
	function eiddo_qodef_header_class( $classes ) {
		...
		$disable_menu_area_shadow = eiddo_qodef_get_meta_field_intersect( 'menu_area_shadow', $id ) == 'no';
		if ( $disable_menu_area_shadow ) {
			$classes[] = 'qodef-menu-area-shadow-disable';
		}
		
		$disable_menu_area_grid_shadow = eiddo_qodef_get_meta_field_intersect( 'menu_area_in_grid_shadow', $id ) == 'no';
		if ( $disable_menu_area_grid_shadow ) {
			$classes[] = 'qodef-menu-area-in-grid-shadow-disable';
		}
		
		$disable_menu_area_border = eiddo_qodef_get_meta_field_intersect( 'menu_area_border', $id ) == 'no';
		if ( $disable_menu_area_border ) {
			$classes[] = 'qodef-menu-area-border-disable';
		}
		
		$disable_menu_area_grid_border = eiddo_qodef_get_meta_field_intersect( 'menu_area_in_grid_border', $id ) == 'no';
		if ( $disable_menu_area_grid_border ) {
			$classes[] = 'qodef-menu-area-in-grid-border-disable';
		}
		
		if ( eiddo_qodef_get_meta_field_intersect( 'menu_area_in_grid', $id ) == 'yes' &&
		     eiddo_qodef_get_meta_field_intersect( 'menu_area_grid_background_color', $id ) !== '' &&
		     eiddo_qodef_get_meta_field_intersect( 'menu_area_grid_background_transparency', $id ) !== '0'
		) {
			$classes[] = 'qodef-header-menu-area-in-grid-padding';
		}
		
		$disable_logo_area_border = eiddo_qodef_get_meta_field_intersect( 'logo_area_border', $id ) == 'no';
		if ( $disable_logo_area_border ) {
			$classes[] = 'qodef-logo-area-border-disable';
		}
		
		$disable_logo_area_grid_border = eiddo_qodef_get_meta_field_intersect( 'logo_area_in_grid_border', $id ) == 'no';
		if ( $disable_logo_area_grid_border ) {
			$classes[] = 'qodef-logo-area-in-grid-border-disable';
		}
		
		if ( eiddo_qodef_get_meta_field_intersect( 'logo_area_in_grid', $id ) == 'yes' &&
		     eiddo_qodef_get_meta_field_intersect( 'logo_area_grid_background_color', $id ) !== '' &&
		     eiddo_qodef_get_meta_field_intersect( 'logo_area_grid_background_transparency', $id ) !== '0'
		) {
			$classes[] = 'qodef-header-logo-area-in-grid-padding';
		}
		
		$disable_shadow_vertical = eiddo_qodef_get_meta_field_intersect( 'vertical_header_shadow', $id ) == 'no';
		if ( $disable_shadow_vertical ) {
			$classes[] = 'qodef-header-vertical-shadow-disable';
		}
		
		$disable_border_vertical = eiddo_qodef_get_meta_field_intersect( 'vertical_header_border', $id ) == 'no';
	...
	add_filter( 'body_class', 'eiddo_qodef_header_class' );
}

if ( ! function_exists( 'eiddo_qodef_header_area_style' ) ) {
	/**
	 * Function that return styles for header area
	 */
	function eiddo_qodef_header_area_style( $style ) {
		...
		$menu_area_background_color_rgba = eiddo_qodef_rgba_color( $menu_area_background_color, $menu_area_background_transparency );
		...
		if ( $container_side_padding !== '' ) {
			...
			$current_style .= eiddo_qodef_dynamic_css( $menu_container_selector, $menu_container_styles );
		}
		...
		$menu_area_grid_background_color_rgba = eiddo_qodef_rgba_color( $menu_area_grid_background_color, $menu_area_grid_background_transparency );
		
		if ( ! empty( $menu_area_grid_background_color_rgba ) ) {
			$menu_area_grid_style['background-color'] = $menu_area_grid_background_color_rgba;
		}
		
		$current_style .= eiddo_qodef_dynamic_css( $menu_area_selector, $menu_area_style );
		$current_style .= eiddo_qodef_dynamic_css( $menu_area_grid_selector, $menu_area_grid_style );
		...
		if ( $dropdown_top_position !== '' ) {
			$dropdown_styles['top'] = eiddo_qodef_filter_suffix( $dropdown_top_position, '%' ) . '%';
		}
		
		$dropdown_selector = array( $class_prefix . ' .qodef-page-header .qodef-drop-down .second' );
		
		$current_style .= eiddo_qodef_dynamic_css( $dropdown_selector, $dropdown_styles );
		...
		$logo_area_background_color_rgba = eiddo_qodef_rgba_color( $logo_area_background_color, $logo_area_background_transparency );
		...
		$logo_area_grid_background_color_rgba = eiddo_qodef_rgba_color( $logo_area_grid_background_color, $logo_area_grid_background_transparency );
		...
		if ( ! empty( $logo_area_style ) ) {
			$current_style .= eiddo_qodef_dynamic_css( $logo_area_selector, $logo_area_style );
		}
		
		if ( ! empty( $logo_area_grid_style ) ) {
			$current_style .= eiddo_qodef_dynamic_css( $logo_area_grid_selector, $logo_area_grid_style );
		}
		
		$current_style = apply_filters( 'eiddo_qodef_add_header_page_custom_style', $current_style, $class_prefix, $page_id ) . $style;
		
		return $current_style;
	}
	
	add_filter( 'eiddo_qodef_add_page_custom_style', 'eiddo_qodef_header_area_style' );
}