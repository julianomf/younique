...
namespace EiddoQodef\Modules\Header\Lib;
/**
 * Class EiddoQodefHeaderConnector
 *
...
class EiddoQodefHeaderConnector {
	...
	public function connect( $config = array() ) {
		do_action( 'eiddo_qodef_pre_header_connect' );
		...
		if ( ! empty( $config['affect_content'] ) ) {
			add_filter( 'eiddo_qodef_content_elem_style_attr', array( $this, 'contentMarginFilter' ) );
		}
		
		if ( ! empty( $config['affect_title'] ) ) {
			add_filter( 'eiddo_qodef_title_content_padding', array( $this, 'titlePaddingFilter' ) );
		}
		
		do_action( 'eiddo_qodef_after_header_connect' );
	}
	 ...
	 *
	 * Hooks to eiddo_qodef_title_content_padding filter
	 *
	 * @return int
	 */
