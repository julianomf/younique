...
class EiddoQodefLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_eiddo_qodef_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_eiddo_qodef_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'eiddo' );
		
		if ( isset( $_COOKIE[ 'qodef-like_' . $post->ID ] ) ) {
			$class = 'qodef-like liked';
			$title = esc_html__( 'You already like this!', 'eiddo' );
		}
...
if ( ! function_exists( 'eiddo_qodef_create_like' ) ) {
	function eiddo_qodef_create_like() {
		EiddoQodefLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'eiddo_qodef_create_like' );
}