...
class EiddoQodefClassIconsGroupWidget extends EiddoQodefWidget {
	public function __construct() {
		parent::__construct(
			'qodef_social_icons_group_widget',
			esc_html__( 'Select Social Icons Group Widget', 'eiddo' ),
			array( 'description' => esc_html__( 'Use this widget to add a group of up to 6 social icons to a widget area.', 'eiddo' ) )
		);
		...
		$this->params = array_merge(
			array(
				...
				array(
					...
					'title' => esc_html__( 'Widget Description', 'eiddo' )
				)
			),
			eiddo_qodef_icon_collections()->getSocialIconWidgetMultipleParamsArray(6),
			array(
				array(
					'type'    => 'dropdown',
					'name'    => 'layout',
					'title'   => esc_html__( 'Icons Layout', 'eiddo' ),
					'options' => array(
						''             => esc_html__( 'Default', 'eiddo' ),
						'square-icons' => esc_html__( 'Square', 'eiddo' ),
					)
				),
				array(
					'type'        => 'dropdown',
					'name'        => 'skin',
					'title'       => esc_html__( 'Square Icons Skin', 'eiddo' ),
					'description' => esc_html__( 'This applies to the square layout', 'eiddo' ),
					'options'     => array(
						''           => esc_html__( 'Dark Skin', 'eiddo' ),
						'light-skin' => esc_html__( 'Light Skin', 'eiddo' ),
					)
				),
				array(
					'type'    => 'dropdown',
					'name'    => 'alignment',
					'title'   => esc_html__( 'Text Alignment', 'eiddo' ),
					'options' => array(
				...
				array(
					'type'  => 'textfield',
					'name'  => 'icon_size',
					'title' => esc_html__( 'Icons Size (px)', 'eiddo' )
				),
				array(
					'type'  => 'colorpicker',
					'name'  => 'color',
	...
	public function widget( $args, $instance ) {
		...
		if ( ! empty( $instance['icon_size'] ) ) {
			$icon_styles[] = 'font-size: ' . eiddo_qodef_filter_px( $instance['icon_size'] ) . 'px';
		}
		...
		if ( ! empty( $instance['widget_description'] ) ) {
			echo '<p class="qodef-social-icons-group-description">' . esc_html( $instance['widget_description'] ) . '</p>';
		}
		...
		for ( $n = 1; $n <= 6; $n++ ) {
			...
			<?php if ( ! empty( $icon_holder_html ) ) { ?>
				<a class="qodef-social-icon-widget-holder qodef-icon-has-hover" <?php echo eiddo_qodef_get_inline_attr( $hover_color, 'data-hover-color' ); ?> <?php eiddo_qodef_inline_style( $icon_styles ) ?>
