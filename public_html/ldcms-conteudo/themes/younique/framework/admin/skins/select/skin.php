...
class SelectSkin extends EiddoQodefSkinAbstract {
    /**
     * Skin constructor. Hooks to eiddo_qodef_admin_scripts_init and eiddo_qodef_enqueue_admin_styles
     */
	public function __construct() {
		$this->skinName = 'select';

        //hook to admin register scripts
        add_action('eiddo_qodef_admin_scripts_init', array($this, 'registerStyles'));
        add_action('eiddo_qodef_admin_scripts_init', array($this, 'registerScripts'));

        //hook to admin enqueue scripts
        add_action('eiddo_qodef_enqueue_admin_styles', array($this, 'enqueueStyles'));
        add_action('eiddo_qodef_enqueue_admin_scripts', array($this, 'enqueueScripts'));

        add_action('eiddo_qodef_enqueue_meta_box_styles', array($this, 'enqueueStyles'));
        add_action('eiddo_qodef_enqueue_meta_box_scripts', array($this, 'enqueueScripts'));
	...
	public function registerScripts() {
		...
		foreach ( $this->scripts as $scriptHandle => $script ) {
			eiddo_qodef_register_skin_script( $scriptHandle, $script['path'], $script['dependency'] );
		}
	}
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            eiddo_qodef_register_skin_style($styleHandle, $stylePath);
        }
    }
    /**
	...
	public function renderOptions() {
        ...
        $tab    = eiddo_qodef_get_admin_tab();
        $active_page = $eiddo_qodef_Framework->qodefOptions->getAdminPageFromSlug($tab);
     ...
     *
     * @see EiddoQodefSkinAbstract::loadTemplatePart()
     */
     ...
     *
	 * @param EiddoQodefAdminPage $page current page to load
     *
     * @see EiddoQodefSkinAbstract::loadTemplatePart()
     */
