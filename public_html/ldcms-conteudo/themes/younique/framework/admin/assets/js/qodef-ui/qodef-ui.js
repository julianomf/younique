(function($){
	...
	function qodefInitGalleryUploader() {
		...
		wp.media.customlibEditGallery1 = {
			frame: function () {
				...
				this._frame.on('update', function () {
					...
					jQuery.ajax({
						type: "post",
						url: ajaxurl,
						data: "action=eiddo_qodef_gallery_upload_get_images&ids=" + ids,
			...
			// in ldcms-includes/js/media-editor.js.source.html
			select: function () {
	...
	function qodefInitAjaxForm() {
		...
		$(document).delegate(".qodef_ajax_form", "submit", function (a) {
			var b = $(this),
				c = {
					action: "eiddo_qodef_save_options"
				};
			...
			qodefImportBtn.on('click', function (e) {
				...
				if (confirm(confirmMessage)) {
					...
					if (import_opt === 'content') {
						for (var i = 1; i < 10; i++) {
							var str = i < 10 ? 'eiddo_content_0' + i + '.xml' : 'eiddo_content_' + i + '.xml';
						...
						for (var i = 1; i < 10; i++) {
							var str = i < 10 ? str = 'eiddo_content_0' + i + '.xml' : 'eiddo_content_' + i + '.xml';
