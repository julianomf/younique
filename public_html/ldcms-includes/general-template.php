...
function wp_logout_url($redirect = '') {
	...
	$logout_url = add_query_arg($args, site_url('ldcms-login.php', 'login'));
...
function wp_login_url($redirect = '', $force_reauth = false) {
	$login_url = site_url('ldcms-login.php', 'login');
...
function wp_registration_url() {
	 ...
	 */
	return apply_filters( 'register_url', site_url( 'ldcms-login.php?action=register', 'login' ) );
}
...
function wp_login_form( $args = array() ) {
	...
	$form = '
		<form name="' . $args['form_id'] . '" id="' . $args['form_id'] . '" action="' . esc_url( site_url( 'ldcms-login.php', 'login_post' ) ) . '" method="post">
			' . $login_form_top . '
...
function wp_lostpassword_url( $redirect = '' ) {
	...
	$lostpassword_url = add_query_arg( $args, network_site_url('ldcms-login.php', 'login') );
			...
			foreach ( (array) $results as $result ) {
				if ( $result->post_date != '0000-00-00 00:00:00' ) {
					...
					if ( $result->post_title ) {
						/** This filter is documented in ldcms-includes/post-template.php */
...
function wp_resource_hints() {
	 ...
	 */
	/** This filter is documented in ldcms-includes/formatting.php */
 ...
 * See wp.editor.initialize() in backend/js/editor.js for initialization options.
 *
 * @uses _WP_Editors
 * @since 4.8.0
 */
 ...
 * @since 2.3.0
 *
 * @param string $file file relative to backend/ without its ".css" extension.
 * @return string
 */
function wp_admin_css_uri( $file = 'backend' ) {
 ...
 * $file is a file relative to backend/ without its ".css" extension. A
 * stylesheet link to that generated URL is printed.
 *
 * @since 2.3.0
 *
 * @param string $file       Optional. Style handle name or file name (without ".css" extension) relative
 * 	                         to backend/. Defaults to 'backend'.
 * @param bool   $force_echo Optional. Force the stylesheet link to be printed rather than enqueued.
 */
function wp_admin_css( $file = 'backend', $force_echo = false ) {
	 ...
	 *                                relative to backend/. Defaults to 'backend'.
	 */
