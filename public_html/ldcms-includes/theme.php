	...
	foreach ( $wp_theme_directories as $theme_root ) {
		...
		foreach ( $dirs as $dir ) {
			...
			if ( file_exists( $theme_root . '/' . $dir . '/style.css' ) ) {
				// ldcms-conteudo/themes/a-single-theme
				// ldcms-conteudo/themes is $theme_root, a-single-theme is $dir
				...
				$found_theme = false;
				// ldcms-conteudo/themes/a-folder-of-themes/*
				// ldcms-conteudo/themes is $theme_root, a-folder-of-themes is $dir, then themes are $sub_dirs
...
function switch_theme( $stylesheet ) {
	...
	if ( is_admin() && false === get_option( 'theme_mods_' . $stylesheet ) ) {
		 ...
		 * the Customizer when accessing backend/widgets.php.
		 */
		if ( 'wp_ajax_customize_save' === current_action() ) {
...
function get_theme_mod( $name, $default = false ) {
	...
	/** This filter is documented in ldcms-includes/theme.php */
...
function _custom_header_background_just_in_time() {
	...
	if ( current_theme_supports( 'custom-header' ) ) {
		...
		if ( is_admin() ) {
			require_once( ABSPATH . 'backend/custom-header.php' );
	...
	if ( current_theme_supports( 'custom-background' ) ) {
		...
		if ( is_admin() ) {
			require_once( ABSPATH . 'backend/custom-background.php' );
...
function check_theme_switched() {
	if ( $stylesheet = get_option( 'theme_switched' ) ) {
		...
		if ( $old_theme->exists() ) {
			...
			/** This action is documented in ldcms-includes/theme.php */
...
function _wp_customize_publish_changeset( $new_status, $old_status, $changeset_post ) {
	...
	if ( ! did_action( 'customize_register' ) ) {
		...
		/** This filter is documented in /ldcms-includes/class-wp-customize-manager.php */
