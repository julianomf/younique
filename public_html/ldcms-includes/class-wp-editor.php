 ...
 * @since 3.3.0
 *
 * Private, not included by default. See wp_editor() in ldcms-includes/general-template.php.
 */
...
final class _WP_Editors {
		...
		if ( ! empty( $buttons ) || $set['media_buttons'] ) {
			...
			if ( $set['media_buttons'] ) {
				...
				if ( ! function_exists( 'media_buttons' ) )
					include( ABSPATH . 'backend/includes/media.php' );
		...
		if ( self::$this_tinymce ) {
			...
			if ( empty( self::$first_init ) ) {
				...
				if ( $set['teeny'] ) {
					 ...
					 * 'myplugin' => 'http://mysite.com/ldcms-conteudo/plugins/myfolder/mce_plugin.js'.
					 *
	...
	public static function enqueue_default_editor() {
		...
		// Also add ldcms-includes/css/editor.css
	...
	private static function default_settings() {
		...
		$settings['content_css'] = includes_url( "css/dashicons$suffix.css?$version" ) . ',' .
			includes_url( "js/tinymce/skins/wordpress/ldcms-conteudo.css?$version" );
