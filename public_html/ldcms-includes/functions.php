...
function do_robots() {
	...
	if ( '0' == $public ) {
		...
		$output .= "Disallow: $path/backend/\n";
		$output .= "Allow: $path/backend/admin-ajax.php\n";
	}
 ...
 * It may be overridden by the WP_TEMP_DIR constant in your sys4.9.4/ldcms-config.php file.
 *
 * @since 2.5.0
 *
...
function _wp_upload_dir( $time = null ) {
	...
	if ( empty( $upload_path ) || 'arquivos' == $upload_path ) {
	...
	if ( !$url = get_option( 'upload_url_path' ) ) {
		if ( empty($upload_path) || ( 'arquivos' == $upload_path ) || ( $upload_path == $dir ) )
	...
	if ( is_multisite() && ! ( is_main_network() && is_main_site() && defined( 'MULTISITE' ) ) ) {
		...
		if ( ! get_site_option( 'ms_files_rewriting' ) ) {
			 ...
			 * had arquivos for the main site.)
			 */
			 ...
			 * 1) We are not on the main site in a post-MU network, as arquivos is used
			 *    there, and
...
function wp_unique_filename( $dir, $filename, $unique_filename_callback = null ) {
	...
	/** This filter is documented in ldcms-includes/functions.php */
...
function wp_upload_bits( $name, $deprecated, $bits, $time = null ) {
	...
	/** This filter is documented in backend/includes/file.php */
 ...
 *
 * If a file exists in the ldcms-conteudo directory named db-error.php, then it will
 ...
 *
 * Will remove backend links to retrieve only return URLs not in the backend
 * directory.
 *
 * @since 2.6.0
 *
...
function wp_guess_url() {
	if ( defined('WP_SITEURL') && '' != WP_SITEURL ) {
		...
		if ( strpos( $_SERVER['REQUEST_URI'], 'backend' ) !== false || strpos( $_SERVER['REQUEST_URI'], 'ldcms-login.php' ) !== false ) {
			$path = preg_replace( '#/(backend/.*|ldcms-login.php)#i', '', $_SERVER['REQUEST_URI'] );
