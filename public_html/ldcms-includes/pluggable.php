...
function wp_mail( $to, $subject, $message, $headers = '', $attachments = array() ) {
	...
	try {
		...
		/** This filter is documented in ldcms-includes/pluggable.php */
...
function auth_redirect() {
	...
	if ( $secure && !is_ssl() && false !== strpos($_SERVER['REQUEST_URI'], 'backend') ) {
	...
	if ( $user_id = wp_validate_auth_cookie( '',  $scheme) ) {
		...
		if ( !$secure && get_user_option('use_ssl', $user_id) && false !== strpos($_SERVER['REQUEST_URI'], 'backend') ) {
			if ( 0 === strpos( $_SERVER['REQUEST_URI'], 'http' ) ) {
				wp_redirect( set_url_scheme( $_SERVER['REQUEST_URI'], 'https' ) );
				exit();
			} else {
 ...
 * list.
 *
 * If the host is not allowed, then the redirect defaults to backend on the siteurl
...
function wp_new_user_notification( $user_id, $deprecated = null, $notify = '' ) {
	...
	$message .= '<' . network_site_url("ldcms-login.php?action=rp&key=$key&login=" . rawurlencode($user->user_login), 'login') . ">\r\n\r\n";
 ...
 * in the database and in the sys4.9.4/ldcms-config.php file. The secret key in the database
 * is randomly generated and will be appended to the secret keys in sys4.9.4/ldcms-config.php.
 *
 * The secret keys in sys4.9.4/ldcms-config.php should be updated to strong, random keys to maximize
 * security. Below is an example of how the secret key constants are defined.
 * Do not paste this example directly into sys4.9.4/ldcms-config.php. Instead, have a
 ...
 * @since 2.5.0
 *
 * @link https://api.wordpress.org/secret-key/1.1/salt/ Create secrets for sys4.9.4/ldcms-config.php
 *
