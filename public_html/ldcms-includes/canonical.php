...
function redirect_canonical( $requested_url = null, $do_redirect = true ) {
	...
	// If we're not in backend and the post has been published and preview nonce
	...
	if ( is_404() ) {
		...
		if ( 'wp-register.php' == basename( $redirect['path'] ) ) {
			if ( is_multisite() ) {
				/** This filter is documented in ldcms-login.php */
 ...
 *
 * If a user visits example.com/admin, they'll be redirected to /backend.
 * Visiting /login redirects to /ldcms-login.php, and so on.
 *
 * @since 3.4.0
 *
 * @global WP_Rewrite $wp_rewrite
...
function wp_redirect_admin_locations() {
	...
	$logins = array(
		home_url( 'ldcms-login.php', 'relative' ),
