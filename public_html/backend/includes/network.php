...
<?php
		...
		if ( WP_CONTENT_DIR != ABSPATH . 'ldcms-conteudo' && ( allow_subdirectory_install() || ! allow_subdomain_install() ) )
			echo '<div class="error inline"><p><strong>' . __( 'Warning:' ) . '</strong> ' . __( 'Subdirectory networks may not be fully compatible with custom ldcms-conteudo directories.' ) . '</p></div>';
...
function network_step2( $errors = false ) {
	...
	if ( ! file_exists( ABSPATH . 'sys4.9.4/ldcms-config.php' ) && file_exists( dirname( ABSPATH ) . '/sys4.9.4/ldcms-config.php' ) ) {
...
?>
		...
		<div class="updated inline"><p><?php
			if ( file_exists( $home_path . '.htaccess' ) ) {
				...
				printf(
					/* translators: 1: sys4.9.4/ldcms-config.php 2: .htaccess */
				...
				echo '<strong>' . __( 'Caution:' ) . '</strong> ';
				printf(
					/* translators: 1: sys4.9.4/ldcms-config.php 2: web.config */
					__( 'We recommend you back up your existing %1$s and %2$s files.' ),
					'<code>sys4.9.4/ldcms-config.php</code>',
				...
				echo '<strong>' . __( 'Caution:' ) . '</strong> ';
				printf(
					/* translators: 1: sys4.9.4/ldcms-config.php */
...
?>
		<ol>
			<li><p><?php printf(
				/* translators: 1: sys4.9.4/ldcms-config.php 2: location of wp-config file, 3: translated version of "That's all, stop editing! Happy blogging." */
        ...
        <rewrite>
            <rules>
                ...
                <rule name="WordPress Rule 2" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . 'backend$" ignoreCase="false" />
                    <action type="Redirect" url="' . $iis_subdir_replacement . 'backend/" redirectType="Permanent" />
                </rule>
...
';
		...
		echo '</p>';
		if ( ! $subdomain_install && WP_CONTENT_DIR != ABSPATH . 'ldcms-conteudo' )
			echo '<p><strong>' . __( 'Warning:' ) . ' ' . __( 'Subdirectory networks may not be fully compatible with custom ldcms-conteudo directories.' ) . '</strong></p>';
		?>
...
{$ms_files_rewriting}
# add a trailing slash to /backend
RewriteRule ^{$subdir_match}backend$ {$subdir_replacement_01}backend/ [R=301,L]
