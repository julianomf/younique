...
function _unzip_file_pclzip($file, $to, $needed_dirs = array()) {
	...
	require_once(ABSPATH . 'backend/includes/class-pclzip.php');
...
function WP_Filesystem( $args = false, $context = false, $allow_relaxed_file_ownership = false ) {
	...
	require_once(ABSPATH . 'backend/includes/class-wp-filesystem-base.php');
	...
	if ( ! class_exists( "WP_Filesystem_$method" ) ) {
		 ...
		 */
		$abstraction_file = apply_filters( 'filesystem_method_file', ABSPATH . 'backend/includes/class-wp-filesystem-' . $method . '.php', $method );
 ...
 *
 * The return value can be overridden by defining the `FS_METHOD` constant in `sys4.9.4/ldcms-config.php`,
 * or filtering via {@see 'filesystem_method'}.
 *
 * @link https://codex.wordpress.org/Editing_sys4.9.4/ldcms-config.php#WordPress_Upgrade_Constants
 *
...
function get_filesystem_method( $args = array(), $context = '', $allow_relaxed_file_ownership = false ) {
	...
	// If the directory doesn't exist (ldcms-conteudo/languages) then use the parent directory as we'll create it.
