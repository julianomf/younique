<?php
/**
 * Retrieves and creates the sys4.9.4/ldcms-config.php file.
 *
 * The permissions for the base directory must allow for writing files in order
 * for the sys4.9.4/ldcms-config.php to be created using this page.
 *
 * @package WordPress
...
require_once( ABSPATH . 'backend/includes/translation-install.php' );
...
// Check if sys4.9.4/ldcms-config.php has been created
if ( file_exists( ABSPATH . 'sys4.9.4/ldcms-config.php' ) ) {
	wp_die( '<p>' . sprintf(
			/* translators: 1: sys4.9.4/ldcms-config.php 2: install.php */
			...
			'<code>sys4.9.4/ldcms-config.php</code>',
			'install.php'
		) . '</p>'
	);
}

// Check if sys4.9.4/ldcms-config.php exists above the root directory but is not part of another installation
if ( @file_exists( ABSPATH . '../sys4.9.4/ldcms-config.php' ) && ! @file_exists( ABSPATH . '../wp-settings.php' ) ) {
	wp_die( '<p>' . sprintf(
			/* translators: 1: sys4.9.4/ldcms-config.php 2: install.php */
...
/**
 * Display setup sys4.9.4/ldcms-config.php file header.
 *
 * @ignore
 * @since 2.3.0
 *
...
<p><?php
	/* translators: %s: sys4.9.4/ldcms-config.php */
	printf( __( 'We&#8217;re going to use this information to create a %s file.' ),
		'<code>sys4.9.4/ldcms-config.php</code>'
	);
	?>
	<strong><?php
		/* translators: 1: wp-config-sample.php, 2: sys4.9.4/ldcms-config.php */
	...
	printf( __( 'Need more help? <a href="%s">We got it</a>.' ),
		__( 'https://codex.wordpress.org/Editing_sys4.9.4/ldcms-config.php' )
	);
?></p>
...
<p><?php
	/* translators: %s: sys4.9.4/ldcms-config.php */
	printf( __( 'Sorry, but I can&#8217;t write the %s file.' ), '<code>sys4.9.4/ldcms-config.php</code>' );
?></p>
<p><?php
	/* translators: %s: sys4.9.4/ldcms-config.php */
	printf( __( 'You can create the %s file manually and paste the following text into it.' ), '<code>sys4.9.4/ldcms-config.php</code>' );
?></p>
...
<?php
		 ...
		 */
		if ( file_exists( ABSPATH . 'wp-config-sample.php' ) )
			$path_to_wp_config = ABSPATH . 'sys4.9.4/ldcms-config.php';
		else
			$path_to_wp_config = dirname( ABSPATH ) . '/sys4.9.4/ldcms-config.php';
