	...
	switch ( $_GET['action'] ) {
		case 'deleteuser':
			...
			if ( $id != '0' && $id != '1' ) {
				...
				echo '</div>';
				require_once( ABSPATH . 'backend/admin-footer.php' );
			} else {
		...
		case 'allusers':
			...
			if ( ( isset( $_POST['action']) || isset($_POST['action2'] ) ) && isset( $_POST['allusers'] ) ) {
				...
				if ( ! in_array( $doaction, array( 'delete', 'spam', 'notspam' ), true ) ) {
					...
					/** This action is documented in backend/network/site-themes.php */
...
<?php require_once( ABSPATH . 'backend/admin-footer.php' ); ?>
